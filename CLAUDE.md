...
- 包结构（`src/main/java/com/minispring`）：
  ...
  - `condition/`：条件装配。`@Conditional`/`Condition`/`ConditionContext`、`ConditionEvaluator`（元注解解析 + AND 求值）、`@ConditionalOnProperty`/`OnPropertyCondition`。`scanComponents` 据此跳过不满足条件的 Bean。
  - `i18n/`：国际化。`MessageSource`/`NoSuchMessageException`/`AbstractMessageSource`、`StaticMessageSource`/`ResourceBundleMessageSource`；`DefaultBeanContainer.setMessageSource` 配置、`@Autowired` 注入。
  - `async/`：异步。`@Async`（方法注解）、`AsyncInterceptor`（`AroundAdvice`，提交 `Executor`）；经 `addAdvisor` 接入，接口方法经代理异步执行（受零依赖所限无真实 CGLIB，仅接口方法）。
  - `transaction/`：事务。`@Transactional`/`Propagation`/`TransactionStatus`/`PlatformTransactionManager`、`SimpleTransactionManager`（ThreadLocal 事务栈，逻辑事务无真实 DB）、`TransactionInterceptor`（`AroundAdvice`，事务边界 + 回滚规则）；经 `addAdvisor` 接入。注意：与阶段 5 的 `aop.interceptor.TransactionInterceptor`（打印桩）同名不同包。
  - `samples/`：每阶段一个 `*Demo` 入口（`ioc`/`di`/`lifecycle`/`annotation`/`aop`/`mvc`/`event`/`conditional`/`messagesource`/`async`/`transaction`）。
...
阶段 7 全部高级特性（事件、条件装配、国际化、异步、事务）已完成。后续可选增强（各特性的 YAGNI 项）：`@EventListener` 注解式、真实 DB 事务（接入 JDBC/DataSource）、其余事务传播行为（SUPPORTS/NESTED 等）、`@ConditionalOnClass`/`@ConditionalOnBean`、`LocaleResolver`、`@Async("executorName")` 等。沿用既有节奏：brainstorming（spec）→ writing-plans（plan）→ subagent-driven / TDD 实现。

> 注：阶段 7-5 事务实现期间发现并修复了一个潜伏的 AOP 缺陷——代理 `proceed()` 未解包 `InvocationTargetException`，使异常感知型 advice（如事务回滚判定）失效（commit 82cd35b）。
