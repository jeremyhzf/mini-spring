...
- 包结构（`src/main/java/com/minispring`）：
  ...
  - `samples/`：每阶段一个 `*Demo` 入口（`ioc`/`di`/`lifecycle`/`annotation`/`aop`/`mvc`/`event`/`conditional`）。
...
- `docs/superpowers/specs/` 与 `docs/superpowers/plans/` —— 新特性的设计 spec 与实现计划（阶段 7-1 事件机制、7-2 条件装配均经 brainstorming → spec → plan → TDD 实现 流程产出，可作后续特性模板）
